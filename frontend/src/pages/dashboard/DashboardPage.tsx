        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-600 text-sm mt-1 font-medium">Bem-vindo de volta! Aqui está o resumo financeiro de hoje.</p>
                </div>
                <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <Link to="/reports" className="px-3 py-2 text-slate-600 hover:text-primary-600 text-xs font-black uppercase tracking-wider transition-colors">Relatórios</Link>
                    {isAdmin && <Link to="/settings" className="px-3 py-2 text-slate-600 hover:text-primary-600 text-xs font-black uppercase tracking-wider transition-colors">Configurações</Link>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden flex flex-col shadow-sm">
                    <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                        ...
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-100">Últimas 5</span>
                    </div>
                    <div className="p-4 space-y-1">
                        {data.recentSales?.length > 0 ? data.recentSales.map((sale: any) => (
                            <div key={sale.id} className="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-all group border border-transparent hover:border-slate-100">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 group-hover:bg-primary-600 group-hover:text-white group-hover:border-primary-600 transition-all duration-300"><ShoppingCart size={20} /></div>
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">{new Date(sale.soldAt).toLocaleDateString('pt-BR')} · {sale.installmentCount} parcelas</p>
                                    </div>
                                </div>
                        ...
                        )) : <div className="text-center py-16 text-slate-400 flex flex-col items-center gap-3"><ShoppingCart size={40} className="opacity-20" /><p className="text-sm font-black uppercase tracking-widest opacity-40">Nenhuma venda registrada</p></div>}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden flex flex-col shadow-sm">
                    <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                        ...
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-100">Próximos 30 dias</span>
                    </div>
                    <div className="p-4 space-y-1">
                        {data.upcomingInstallments?.length > 0 ? data.upcomingInstallments.map((inst: any) => (
                            <div key={inst.id} className="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-all group border border-transparent hover:border-slate-100">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-500 transition-all duration-300"><Calendar size={20} /></div>
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">Parcela {inst.number} · Vence em {new Date(inst.dueDate).toLocaleDateString('pt-BR')}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right">
                                    ...
                                    <div className="p-3 bg-slate-50 rounded-xl text-slate-400 group-hover:translate-x-1 group-hover:text-amber-500 group-hover:bg-white group-hover:shadow-md transition-all border border-transparent group-hover:border-slate-100"><ArrowRight size={18} /></div>
                                </div>
                            </div>
                        )) : <div className="text-center py-16 text-slate-400 flex flex-col items-center gap-3"><Clock size={40} className="opacity-20" /><p className="text-sm font-black uppercase tracking-widest opacity-40">Nenhuma parcela pendente</p></div>}
                    </div>
                </div>
            </div>
        </div>
