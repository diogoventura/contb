    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary-500" /></div>
                : sales.length === 0 ? <div className="glass-card p-20 text-center bg-white border-dashed border-2 border-slate-200 shadow-sm"><ShoppingCart size={48} className="mx-auto text-slate-400 mb-4" /><p className="text-slate-500 font-bold">Nenhuma venda realizada</p></div>
                    : (
                                ...
                                <div key={s.id} className={`glass-card overflow-hidden group border transition-all duration-300 bg-white shadow-sm ${expandedId === s.id ? 'ring-2 ring-primary-100 border-primary-200' : 'border-slate-100 hover:border-primary-100 hover:shadow-md'}`}>
                                    <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => setExpandedId(expandedId === s.id ? null : s.id)}>
                                        <div className="flex items-center gap-5">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); openDetails(s); }} className="p-1.5 text-slate-400 hover:text-slate-600 transition-colors" title="Ver detalhes"><Eye size={18} /></button>
                                                </div>
                                                <div className="flex items-center gap-3 text-[11px] font-black text-slate-500 uppercase tracking-widest mt-1">
                                                    <span className="flex items-center gap-1.5"><Calendar size={12} className="text-slate-500" /> {new Date(s.soldAt).toLocaleDateString('pt-BR')}</span>
                                                    ...
                                                    <span className="text-slate-300">/</span>
                                                    <span className="text-slate-600">{s.installmentCount} parcelas</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div className="bg-slate-50 p-2 rounded-full border border-slate-100">
                                                {expandedId === s.id ? <ChevronUp size={20} className="text-primary-600" /> : <ChevronDown size={20} className="text-slate-400" />}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {expandedId === s.id && (
                                        <div className="px-6 pb-6 pt-4 border-t border-slate-100 bg-slate-50 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] animate-in slide-in-from-top-2 duration-300">
                                            {/* Items Section */}
                                            <div className="mb-6">
                                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><Package size={14} className="text-primary-600" /> Itens Comprados</h4>
                                            ...
                                            <div>
                                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><FileText size={14} className="text-amber-500" /> Cronograma de Pagamento</h4>
                                                    ...
                                                    {s.installments?.map((inst: any) => (
                                                        <div key={inst.id} className={`flex items-center justify-between bg-white rounded-xl p-4 border transition-all shadow-sm ${selectedInstallments.includes(inst.id) ? 'border-primary-200 bg-primary-50/30 ring-1 ring-primary-100' : 'border-slate-100 hover:bg-slate-50'}`}>
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[10px] font-black text-slate-500 border border-slate-100">{inst.number}/{s.installmentCount}</div>
                                                                <div>
                                                                    <p className="text-sm font-black text-slate-900">R$ {Number(inst.amount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-tighter mt-0.5">Vencimento: {new Date(inst.dueDate).toLocaleDateString('pt-BR')}</p>
                                                                </div>
                                                            </div>
                                                            <div className="flex items-center gap-4">
                    ...
                    <div className="bg-white w-full max-w-2xl p-8 rounded-3xl space-y-8 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto shadow-2xl border border-white" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs font-black text-slate-500 uppercase tracking-widest mt-1">Preencha os dados do cliente e produtos</p>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                                <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] flex items-center gap-2"><Package size={14} className="text-primary-600" /> Produtos e Itens</h4>
                                ...
                                {form.items.map((item, idx) => (
                                    <div key={idx} className="flex flex-wrap md:flex-nowrap gap-4 items-end bg-slate-50/50 p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
                                        <div className="flex-1 min-w-[200px] space-y-2">
                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Produto</label>
                                        ...
                                        <div className="w-24 space-y-2">
                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Qtd</label>
                                        ...
                                        <div className="w-36 space-y-2">
                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Preço Unitário</label>
                                        ...
                                        <button onClick={() => removeItem(idx)} className="p-3 text-slate-400 hover:text-red-500 bg-white border border-slate-200 rounded-xl transition-all shadow-sm hover:border-red-100"><Trash2 size={18} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-inner">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Total da Venda</label>
                        ...
                        <div className="pt-4 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter ml-2">Lembrar Antes (Dias)</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter ml-2">Aviso Atraso (Intervalo)</label>
                    ...
                    <div className="bg-white w-full max-w-4xl p-8 rounded-3xl space-y-8 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto shadow-2xl border border-white" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-start">
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4">
                                    <p className="text-xs font-black text-slate-500 uppercase tracking-widest">ID: #{selectedSale.id} · Realizada em {new Date(selectedSale.soldAt).toLocaleDateString('pt-BR')}</p>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setShowDetailModal(false)} className="p-2.5 text-slate-500 hover:text-slate-600 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm transition-all hover:rotate-90 ml-3"><X size={24} /></button>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 ml-1"><User size={14} className="text-primary-600" /> Informações do Cliente</h4>
                                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-5 shadow-sm">
                                            <div>
                                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Nome Completo</p>
                                                <p className="text-base font-black text-slate-900 tracking-tight">{selectedSale.personName}</p>
                                            </div>
                                            <div className="space-y-4">
                                                <div>
                                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">WhatsApp</p>
                                                ...
                                                <div>
                                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">E-mail</p>
                                            ...
                                            <div>
                                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Documento/CPF</p>
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 ml-1"><Package size={14} className="text-orange-500" /> Itens da Venda</h4>
                                        <div className="space-y-3">
                                            {selectedSale.saleItems?.map((item: any) => (
                                                <div key={item.id} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 hover:border-primary-200 hover:shadow-md transition-all group">
                                                    <div className="space-y-1">
                                                        ...
                                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.quantity} un. × R$ {Number(item.unitPrice).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                                    </div>
                                                    <p className="text-sm font-black text-emerald-600">R$ {Number(item.subtotal).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                                </div>
                                            ))}
                                ...
                                <div className="space-y-4">
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 ml-1">Observações</h4>
                            ...
                            <div className="space-y-6">
                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 ml-1"><Clock size={14} className="text-primary-600" /> Resumo Financeiro</h4>
