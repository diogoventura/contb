        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1 font-medium">Acompanhe as notificações do sistema e cobranças automáticas.</p>
                </div>
            ...
            {loading ? <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary-600" /></div>
                : notifications.length === 0 ? (
                    <div className="glass-card p-20 text-center bg-white border-dashed border-2 border-slate-200 shadow-sm">
                        ...
                        <p className="text-slate-500 max-w-xs mx-auto font-medium">Você não tem novas notificações no momento. O sistema avisará quando algo precisar da sua atenção.</p>
                    </div>
                ) : (
                            ...
                            <div key={n.id} className={`glass-card p-5 flex items-start gap-5 transition-all group border bg-white shadow-sm hover:shadow-md animate-in slide-in-from-right-4 duration-500 ${!n.isRead ? 'border-primary-100 ring-2 ring-primary-50/50' : 'border-slate-100'}`} style={{ animationDelay: `${idx * 50}ms` }}>
                                ...
                                <div className="flex-1 min-w-0 pr-4">
                                    <div className="flex items-center gap-3 mb-1.5">
                                        <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${!n.isRead ? 'text-primary-600' : 'text-slate-400'}`}>
                                            {n.type || 'Notificação'}
                                        </span>
                                        <span className="w-1 h-1 rounded-full bg-slate-200" />
                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{new Date(n.createdAt).toLocaleString('pt-BR')}</p>
                                    </div>
                                    <p className={`text-sm leading-relaxed tracking-tight ${!n.isRead ? 'text-slate-900 font-bold' : 'text-slate-500 font-medium'}`}>{n.content}</p>
                                </div>
                                <div className="flex flex-col gap-2">
                                    {!n.isRead ? (
                                        <button onClick={() => markRead(n.id)} className="p-2.5 text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all border border-transparent hover:border-emerald-100" title="Marcar como lido">
                                            <Check size={20} />
                                        </button>
                                    ) : (
